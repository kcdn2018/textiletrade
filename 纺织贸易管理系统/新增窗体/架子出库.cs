...
#pragma warning disable CS0234 // 命名空间“Sunny.UI”中不存在类型或命名空间名“UIForm”(是否缺少程序集引用?)
    public partial class 架子出库 : Sunny.UI.UIForm 
#pragma warning restore CS0234 // 命名空间“Sunny.UI”中不存在类型或命名空间名“UIForm”(是否缺少程序集引用?)
    {
        ...
        private void 确认出库ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            foreach (string j in uiTransfer1.ItemsRight)
            {
                ...
                JiaziTableService.UpdateJiaziTable($"State='{txtkehu.Text }'", X => X.JiaziID == jiazi[0]&&X.Guisuo ==jiazi[1]);
                ZhijiaMingxiTableService.InsertZhijiaMingxiTable(new ZhijiaMingxiTable() {  Danhao ="CK"+DateTime.Now .ToString (), Bianhao = JiaziTableService.GetOneJiaziTable(X => X.JiaziID == jiazi[0] && X.Guisuo == jiazi[1]).ID.ToString(), _date =uiDatePicker1.Value , Remarker =txtbeizhu.Text,Leixing ="出库",
                Kehu =txtkehu.Text });
            }
              导入在库的子架号();
            txtbeizhu.Text = "";
