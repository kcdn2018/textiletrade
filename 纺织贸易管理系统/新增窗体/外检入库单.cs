...
{
    public partial class 外检入库单 : Form
    {
        ...
        private void 加载卷()
        {
            ...
            {
                juanList.AddRange(JuanHaoTableService.GetJuanHaoTablelst(x => x.Ckmc == txtkehu .Text && x.CustomerName == d.CustomName  && x.SampleName == d.pingming && x.yanse == d.yanse && x.GangHao == d.ganghao && x.state == 0
                && x.kuanhao == d.kuanhao && x.OrderNum == d.OrderNum && x.SampleNum == x.SampleNum && x.Houzhengli == d.houzhengli && x.Huahao == d.Huahao).OrderBy (x=>x.PiHao ));
            }
        ...
        private void 保存ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            { ////检查库存。没有不能发货
                ...
                var d = 库存BLL.检查库存(danjumingxitables, txtkehu.Text );
                if (d.Bianhao != null)
                {
                ...
                外检入库单BLL.保存单据(danju, danjumingxitables, CreatJuanhao());
            }
            else
            {
                外检入库单BLL.修改单据(danju, danjumingxitables, CreatJuanhao());
            }
        ...
        { 
            ...
            txtdanhao.Text = BianhaoBLL.CreatDanhao(FirstLetter.外检入库单, dateEdit1.DateTime, DanjuLeiXing.外检入库单);
            txtChachefei.Text = "0";
            txtyunfei.Text = "0";
        ...
        private void Edit()
        {
            ...
            juanList = JuanHaoTableService.GetJuanHaoTablelst(x => x.Danhao == txtdanhao.Text).OrderBy (x=>x.GangHao ).ThenBy (x=>x.yanse ).ThenBy (x=>x.PiHao ).ToList ();
            gridControl2.DataSource = juanList;
        }
        ...
        private void 重新加载卷()
        {
            ...
            foreach (var d in danjumingxitables.Where(x => x.Bianhao != null))
            {
              ...
              && x.GangHao == d.ganghao && x.SampleNum == d.Bianhao && x.Huahao == d.Huahao && x.ColorNum == d.ColorNum && x.Ckmc == txtkehu.Text && x.Danhao == txtdanhao.Text));
                juanList.AddRange(JuanHaoTableService.GetJuanHaoTablelst(x => x.OrderNum == d.OrderNum && x.yanse == d.yanse && x.kuanhao == d.kuanhao && x.Houzhengli == d.houzhengli
