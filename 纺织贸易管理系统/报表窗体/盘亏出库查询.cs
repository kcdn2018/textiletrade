...
namespace 纺织贸易管理系统.报表窗体
{
    public partial class 盘亏出库查询 : Form
    {
        ...
        private void 采购查询_Load(object sender, EventArgs e)
        {
            ...
            CreateGrid.Query<DanjuTable>(gridControl1, DanjuTableService.GetDanjuTablelst(x=>x.rq >=Convert.ToDateTime ( dateEdit1.Text)&&x.rq<= Convert.ToDateTime(dateEdit2.Text)&&x.ksmc .Contains (txtksmc.Text )&&x.zhuangtai !="已删除"&&x.djlx==DanjuLeiXing.盘亏出库单  ));
        }
            ...
            if (AccessBLL.CheckAccess("删除盘亏出库单"))
            {
                if ((int)(MessageBox.Show("您确定要删除改盘亏出库单吗？确定按YES.取消按NO", "提示", MessageBoxButtons.YesNo)) == 6)
                {
                    if (单据BLL.检查是否已经审核(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) == false)
                    {
                        ...
                        CreateGrid.Query<DanjuTable>(gridControl1, DanjuTableService.GetDanjuTablelst(x => x.rq >= Convert.ToDateTime(dateEdit1.Text) && x.rq <= Convert.ToDateTime(dateEdit2.Text) && x.zhuangtai != "已删除" && x.djlx == DanjuLeiXing.盘亏出库单));
                    }
                    else
                    {
