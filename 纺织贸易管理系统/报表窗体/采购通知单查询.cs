...
{
    public partial class 采购通知单查询 : Form
    {
        public 采购通知单查询()
        {
            InitializeComponent();
        ...
        private void 刷新ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                gridControl1.DataSource = DanjuTableService.GetDanjuTablelst(x => x.rq >=dateEdit1.DateTime.Date && x.rq <= dateEdit2.DateTime.Date  && x.ksmc.Contains(txtksmc.Text) && x.zhuangtai != "已删除" && x.djlx == DanjuLeiXing.采购通知单);
            }
            else
            {
        ...
        {
            ExportFile.导出到文件(gridControl1,"采购通知单清单");
        }
