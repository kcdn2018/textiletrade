...
{
    public partial class 应收款汇总 : Form
    {
        public 应收款汇总()
        {
            InitializeComponent();
        ...
        private void Query()
        {
            var lxrllist = LXRService.GetLXRlst(x=>x.MC.Contains (txtkehu.Text)&&x.LX=="客户");
            ...
            foreach (var l in lxrllist )
            {
                ...
                drow.Cells[Cbencixiaoshoue.Name].Value = danjumx.Where (x=>x.djlx==DanjuLeiXing.销售出库单 ).Sum(x => x.je);
                //累加本次销售额
                bencixiaoshoue  += Convert.ToDecimal(drow.Cells[Cbencixiaoshoue.Name].Value);
                ////
                drow.Cells[Cbencixiaoshoushu.Name].Value = danjumx.Where(x => x.djlx == DanjuLeiXing.销售出库单).Sum(x => x.TotalMishu);
                ///累计本次销售数
                bencixiaoshou += Convert.ToDecimal(drow.Cells[Cbencixiaoshoushu.Name].Value);
                //
                drow.Cells [Cbenciyingshoukuan .Name ].Value = danjumx.Where(x => x.djlx == DanjuLeiXing.销售出库单||x.djlx==DanjuLeiXing.色卡销售单 ||x.djlx==DanjuLeiXing.寄样单 ).Sum(x => x.je);
                //累计本次应收款
                benciyingshoukuan += Convert.ToDecimal(drow.Cells[Cbenciyingshoukuan.Name].Value);
                //
                drow.Cells[Cbencitiaochong.Name ].Value = danjumx.Where(x => x.djlx == DanjuLeiXing.收款单).Sum(x => x.totalmoney );
                //累计本次调充数
                bencitiaochong += Convert.ToDecimal(drow.Cells[Cbencitiaochong .Name].Value);
                ///
                drow.Cells[Cbenqixvkaipiaoe .Name].Value = danjumx.Where(x =>x.Hanshui=="含税" && x.djlx == DanjuLeiXing.销售出库单 || x.djlx == DanjuLeiXing.色卡销售单 ).Sum(x => x.je);
               ///累计本次需开票额
               benqixvkaipiao += Convert.ToDecimal(drow.Cells[Cbenqixvkaipiaoe .Name].Value);
                ////
                drow.Cells[Cbenqiyikaipiaoe .Name ].Value = danjumx.Where(x => x.djlx==DanjuLeiXing.发票开具 ).Sum(x => x.je);
                ///累计本期已开票额
                ...
                drow.Cells[Cyikaipiaoweifukuan.Name].Value  =remainlist.Where (x=>x.MachineType ==DanjuLeiXing.发票开具 ).Sum (x=>x.ShengYuMoney );
                ////累计已开票未付款
                yikiaopaiaoweifukuan+= Convert.ToDecimal(drow.Cells[Cyikaipiaoweifukuan .Name].Value);
                ////
                var lxr = LXRService.GetOneLXR(y => y.MC == l.MC);
                drow.Cells[Cyuqijine.Name].Value = RemainMoneyTableService.GetRemainMoneyTablelst (x=>x.MachineType ==DanjuLeiXing.发票开具 &&x.ShengYuMoney>0&&x.Rq <=DateTime.Now.AddDays(-lxr.ZhangQi )).Sum(x=>x.ShengYuMoney );
               /////累计逾期款
        ...
        {
            ExcelService.ExportExcels("应收款汇总", dataGridViewX1);
        }
    }
}
