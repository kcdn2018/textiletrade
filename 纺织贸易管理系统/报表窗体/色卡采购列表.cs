...
{
    public partial class 色卡采购列表 : Form 
    {
        public 色卡采购列表()
        {
            InitializeComponent();
        ...
        public virtual  void Query()
        {
            CreateGrid.Query(gridControl1, $"select danjutable.*,danjumingxitable.* from danjutable,danjumingxitable where danjutable.rq between '{ Convert.ToDateTime(dateEdit1.Text)}' and '{Convert.ToDateTime(dateEdit2.Text)}' and danjutable.ksmc like '%{txtksmc.Text}%' " +
                    ...
                    $"and danjutable.djlx='{DanjuLeiXing.色卡采购单  }' "+
        ...
        private void 新增ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MainForm.mainform.AddMidForm(new 色卡采购单 () { useful = FormUseful.新增 });
            }
        }

        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (AccessBLL.CheckAccess("修改色卡采购单") == true)
            {
                if (单据BLL.检查是否已经审核(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) == false)
                {
                    MainForm.mainform.AddMidForm(new 色卡采购单 () { useful = FormUseful.修改, danju = DanjuTableService.GetOneDanjuTable(x => x.dh == gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) });
                }
                else
                {
                    if (SysInfo.GetInfo.own != "审核制")
        ...
        {
            if (AccessBLL.CheckAccess("删除色卡采购单"))
            {
                if ((int)(MessageBox.Show("您确定要删除改色卡采购吗？确定按YES.取消按NO", "提示", MessageBoxButtons.YesNo)) == 6)
                {
                    if (单据BLL.检查是否已经审核(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) == false)
                    {
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                MainForm.mainform.AddMidForm(new 色卡采购单 () { useful = FormUseful.查看, danju = DanjuTableService.GetOneDanjuTable(x => x.dh == gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) });
            }
        }
    }
}
