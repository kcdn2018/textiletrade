...
{
    public partial class 白坯销售列表 : Form
    {
        public 白坯销售列表()
        {
            InitializeComponent();  
        ...
        public virtual void Query()
        {
            ...
            var querystring = $"select danjutable.*,danjumingxitable.* from danjutable,danjumingxitable where danjutable.rq between '{ dateEdit1.DateTime.Date }' and '{dateEdit2.DateTime.Date.AddDays(1) }' and danjutable.ksmc like '%{txtksmc.Text}%' " +
                      ...
                      $"and danjutable.djlx='{DanjuLeiXing.白坯销售单}' " +
        ...
        private void 新增ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MainForm.mainform.AddMidForm(new 白坯销售单() { useful = FormUseful.新增 });
            }
        }

        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (AccessBLL.CheckAccess("修改白坯销售单") == true)
            {
                if (单据BLL.检查是否已经审核(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) == false)
                {
                    MainForm.mainform.AddMidForm(new 白坯销售单() { useful = FormUseful.修改, danju = DanjuTableService.GetOneDanjuTable(x => x.dh == gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) });
                }
                else
                {
                    if (SysInfo.GetInfo.own != "审核制")
        ...
        {
            if (AccessBLL.CheckAccess("删除白坯销售单"))
            {
                if ((int)(MessageBox.Show("您确定要删除改白坯销售单吗？确定按YES.取消按NO", "提示", MessageBoxButtons.YesNo)) == 6)
                {
                    if (单据BLL.检查是否已经审核(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) == false)
                    {
                        白坯销售单BLL.删除单据(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString());
                        Query();
                    }
                    else
                    {
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                MainForm.mainform.AddMidForm(new 白坯销售单() { useful = FormUseful.查看, danju = DanjuTableService.GetOneDanjuTable(x => x.dh == gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) });
            }
        }
