...
{
    public partial class 配桶登记列表 : Form
    {
        public 配桶登记列表()
        {
            InitializeComponent();
        ...
        public virtual void Query()
        {
            ...
            var querystring = $"select danjutable.*,danjumingxitable.* from danjutable,danjumingxitable where danjutable.rq between '{ dateEdit1.DateTime}' and '{dateEdit2.DateTime.Date.AddDays(1)}' and danjutable.ksmc like '%{txtksmc.Text}%' " +
                    ...
                    $"and danjutable.djlx='{DanjuLeiXing.配桶登记单   }' " +
        ...
        private void 新增ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MainForm.mainform.AddMidForm(new 配桶登记单 () { UseFul = FormUseful.新增 });
            }
        }
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (AccessBLL.CheckAccess("修改配桶登记单") == true)
            {
                ...
                {
                    MainForm.mainform.AddMidForm(new 配桶登记单() { UseFul = FormUseful.修改, danju = DanjuTableService.GetOneDanjuTable(x => x.dh == gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) });
                }
            }
        }
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (gridView1.FocusedRowHandle >= 0)
            { 
                if (AccessBLL.CheckAccess("删除配桶登记单"))
                {
                    if ((int)(MessageBox.Show("您确定要删除改配桶登记单吗？确定按YES.取消按NO", "提示", MessageBoxButtons.YesNo)) == 6)
                    {
