...
{
    public partial class 采购退货列表 : Form
    {
        public 采购退货列表()
        {
            InitializeComponent();
        ...
        public virtual void Query()
        {
            var querystring = $"select danjutable.*,danjumingxitable.* from danjutable,danjumingxitable where danjutable.rq between '{ Convert.ToDateTime(dateEdit1.Text)}' and '{Convert.ToDateTime(dateEdit2.Text)}' and danjutable.ksmc like '%{txtksmc.Text}%' " +
                    ...
                    $"and danjutable.djlx='{DanjuLeiXing.采购退货单 }' " +
        ...
        {
            if (AccessBLL.CheckAccess("修改采购退货单") == true)
            {
        ...
        {
            ExportFile.导出到文件(gridControl1, "采购退货单清单");
        }

        private void 审核通过ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (AccessBLL.CheckAccess("审核采购退货单"))
            {
                if (单据BLL.检查是否已经审核(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) == false)
                {
        ...
        {
            if (AccessBLL.CheckAccess("反审核采购退货单"))
            {
