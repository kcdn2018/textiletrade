...
{
    public partial class 委外通知列表 : Form 
    {
        public 委外通知列表()
        {
            InitializeComponent();
        ...
        private void 新增ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MainForm.mainform.AddMidForm(new 委外通知单() { useful = FormUseful.新增 });
            }
        }

        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (AccessBLL.CheckAccess("修改委外通知单") == true)
            {
                MainForm.mainform.AddMidForm(new 委外通知单() { useful = FormUseful.修改, danju = DanjuTableService.GetOneDanjuTable(x => x.dh == gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) });
            }
        }

        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (AccessBLL.CheckAccess("删除委外通知单"))
            {
                if (Sunny.UI.UIMessageBox.ShowAsk ($"您确定要删除单号为{gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()}该委外通知单吗？") )
                {
        ...
        {
            ExportFile.导出到文件(gridControl1, "委外通知单");
        }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                MainForm.mainform.AddMidForm(new 委外通知单() { useful = FormUseful.查看, danju = DanjuTableService.GetOneDanjuTable(x => x.dh == gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) });
            }
        }
        ...
        private void 复制ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (gridView1.FocusedRowHandle >= 0)
            {
                MainForm.mainform.AddMidForm(new 委外通知单() { useful = FormUseful.复制, danju = DanjuTableService.GetOneDanjuTable(x => x.dh == gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "dh").ToString()) });
            }
        }
    }
}
