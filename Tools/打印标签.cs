...
namespace Tools
{
   public static  class 打印标签
    {
        public static void 打印寄样票签(List<LabelExtend >labels, PrintSetting printSetting)
        {
            ...
            {
                dt.Rows.Add();
                var pro = label .GetType().GetProperties();
                foreach (PropertyInfo p in pro)
                {
                    if (collist.Where<ColumnTable>(x => x.DataProperty == p.Name).ToList().Count > 0)
                    {
                        dt.Rows[row][collist.Where<ColumnTable>(x => x.DataProperty == p.Name).ToList()[0].ColumnText] = p.GetValue(label, null);
                    }
                }
                dt.Rows[row]["数量"] = label .数量;
                dt.Rows[row]["合同号"] = label.合同号 ;
                dt.Rows[row]["款号"] = label.款号 ;
                dt.Rows[row]["单位"] = label.单位;
                dt.Rows[row]["特点说明"] =label .Characteristic;
                try
                {
                    dt.Rows[row]["日期"] = label .rq.ToString("d");
                }
                catch
                {
                    dt.Rows[row]["日期"] =DateTime.Now ;
                }
                row++;
            }
