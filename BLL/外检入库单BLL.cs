   ...
   public static  class 外检入库单BLL
    {
        ...
        public static  void 反审核(string danhao)
        {
            ...
            danju.ckmc = danju.ksmc;
            库存BLL.增加库存(danjumingxis, danju);
            var JuanList = JuanHaoTableService.GetJuanHaoTablelst(x => x.Danhao == danhao);
            JuanList.ForEach(x => x.Ckmc = danju.ksmc);
            Connect.DbHelper().Updateable<JuanHaoTable>(JuanList ).ExecuteCommand();
            可发卷BLL.卷入库(danhao);
